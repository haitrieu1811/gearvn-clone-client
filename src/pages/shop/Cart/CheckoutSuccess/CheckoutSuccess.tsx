...
const CheckoutSuccess = () => {
  ...
  return (
    <div className='px-6 md:px-16 py-6'>
      <div className='flex items-center justify-center text-base md:text-lg text-[#1E9800] p-2 md:p-[14px] mb-6 bg-[#D5F7E0] rounded'>
        <ShoppingBagCheckIcon />
        <span className='ml-1 font-semibold'>Đặt hàng thành công</span>
      </div>
      <div className='text-center mb-7 text-sm md:text-base'>
      ...
      <div className='bg-[#ececec] mb-6 pb-4'>
        <div className='p-4 flex justify-between items-center border-b border-b-[#CFCFCF]'>
          <div className='uppercase text-sm md:text-base'>ĐƠN HÀNG {`#${orderId?.slice(-6)}`}</div>
          <Link to={PATH.ACCOUNT_ORDER} className='text-[#1982F9] text-sm md:text-base'>
            Quản lý đơn hàng
          </Link>
        </div>
        ...
        <div className='p-2 md:p-3 rounded border border-dashed border-[#FF7A00] flex justify-center items-center text-[#FF7A00] bg-[#fff6ed] mx-4 text-sm md:text-base'>
          Đơn hàng chưa được thanh toán
        </div>
      </div>
      <Link
        to={PATH.HOME}
        className='mt-4 p-2 md:p-3 rounded text-[#1982f9] font-semibold border border-[#1982f9] text-base md:text-lg flex justify-center items-center hover:bg-[#1982f9] hover:text-white duration-300'
      >
        Tiếp tục mua hàng
      </Link>
    </div>
